...
describe("entitlements", () => {
  ...
  test("trial expirado bloqueia entitlement", () => {
    ...
    expect(() => {
      assertSubscriptionAccess(trial, new Date("2026-09-18T00:00:00Z"));
  ...
  test("feature presente funciona e ausente é bloqueada", () => {
    ...
    expect(() => {
      assertFeature(snapshot(), "reports");
    }).toThrow(EntitlementError);
  });
  ...
  test("limite é retornado e respeitado", () => {
    ...
    expect(() => {
      assertWithinLimit(snapshot(), "max_products", 9, 1);
    }).not.toThrow();
    expect(() => {
      assertWithinLimit(snapshot(), "max_products", 10, 1);
    }).toThrow(EntitlementError);
  });

  test("suspended, canceled e ausência de assinatura bloqueiam", () => {
    expect(() => {
      assertSubscriptionAccess(snapshot({ status: "suspended" }));
    }).toThrow(EntitlementError);
    expect(() => {
      assertSubscriptionAccess(snapshot({ status: "canceled" }));
    }).toThrow(EntitlementError);
    expect(() => {
      assertSubscriptionAccess(null);
    }).toThrow(EntitlementError);
  });
});
