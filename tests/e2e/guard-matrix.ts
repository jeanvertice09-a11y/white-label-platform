...
function deps(over:Partial<RouteDeps>):RouteDeps{return{resolveSession:async()=>null,memberships:{async getPlatformRoles(){return[];},async getTenantMemberships(){return[];}},resolveTenantForHost:async()=>null,getTenantStatus:async()=>"active",...over};}
function userDeps(userId:string,platform:PlatformRole[],tenants:MembershipRow[],hostMap:Record<string,TenantResolution>,status:string|null="active"):RouteDeps{return deps({resolveSession:async()=>stubSession(userId),memberships:{async getPlatformRoles(){return platform;},async getTenantMemberships(){return tenants;}},resolveTenantForHost:async(host)=>hostMap[host]??null,getTenantStatus:async()=>status});}
