...
import { removeStoreMember } from "./control-team.write.server.ts";
import { removeTenantMemberGuarded } from "./control-team-tenant-guard.server.ts";
import { inviteStoreMember, inviteTenantMember } from "./control-team-invite.server.ts";

const tenantRoleSchema = z.enum(["tenant_owner", "tenant_admin", "tenant_finance", "tenant_support"]);
...
export const saveTenantMemberAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    return inviteTenantMember(ctx.sql, ctx.tenantId, ctx.actorUserId, ctx.actorIsOwner, data.email, data.role);
  });
...
export const removeTenantMemberAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const ctx = await controlTeamMutation();
    return removeTenantMemberGuarded(ctx.sql, ctx.tenantId, ctx.actorUserId, ctx.actorIsOwner, data.userId);
  });

export const saveStoreMemberAction = createServerFn({ method: "POST" })
  .validator(z.object({ storeId: z.string().uuid(), email: emailSchema, role: managedStoreRoleSchema }))
  .handler(async ({ data }) => {
    const ctx = await controlTeamMutation();
    return inviteStoreMember(ctx.sql, ctx.tenantId, ctx.actorUserId, data.storeId, data.email, data.role);
  });
...
export const removeStoreMemberAction = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const ctx = await controlTeamMutation();
    return removeStoreMember(ctx.sql, ctx.tenantId, ctx.actorUserId, data.storeId, data.userId);
  });
