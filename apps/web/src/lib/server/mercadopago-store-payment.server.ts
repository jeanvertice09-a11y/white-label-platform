...
export async function createStorePixPayment(scope:Scope,input:{orderId:string;orderNumber:number;amountCents:number;payerEmail:string}):Promise<{paymentId:string;checkout:PaymentCheckoutData}>{
 ...
 const old=existing.at(0);if(old){const oldCheckout={qrCode:typeof old["checkout_qr_code"]==="string"?old["checkout_qr_code"]:null,
 ...
 const usable=Boolean(oldCheckout.qrCode||oldCheckout.ticketUrl)&&(!oldCheckout.expiresAt||Date.parse(oldCheckout.expiresAt)>Date.now());if(usable)return{paymentId:String(old["id"]),checkout:oldCheckout};
 throw new Error("O Pix anterior expirou ou ficou incompleto. Inicie um novo checkout.");}
...
export async function reconcileStoreOrderPayment(scope:Scope,orderId:string):Promise<{status:string;changed:boolean}>{
 const sql=createAdminSqlExecutor();const rows=await sql.query(`select p.id::text,p.provider_payment_id,p.gateway_account_id::text from public.payments p where p.level='store_checkout' and p.tenant_id=$1::uuid and p.store_id=$2::uuid and p.order_id=$3::uuid limit 1`,[scope.tenantId,scope.storeId,orderId]);const row=rows.at(0);if(!row)throw new Error("Pagamento não encontrado.");
 const providerPaymentId=row["provider_payment_id"],gatewayAccountId=row["gateway_account_id"];if(typeof providerPaymentId!=="string"||typeof gatewayAccountId!=="string")throw new Error("Pagamento ainda não possui referência do Mercado Pago.");
 await ensureMercadoPagoGatewayAccessToken(gatewayAccountId);const account=await gateway(scope);if(account.id!==gatewayAccountId)throw new Error("Gateway incompatível com a loja.");const token=await accessToken(account),provider=createPaymentProvider("mercadopago",{credentials:token,webhookSecret:null},{writesEnabled:false});
 const { reconcilePaymentStatus }=await import("@white-label/payments/server");return reconcilePaymentStatus(sql,String(row["id"]),gatewayAccountId,provider,providerPaymentId as ProviderPaymentId);
}
