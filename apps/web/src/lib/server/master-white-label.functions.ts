...
export const createMasterDomain=createServerFn({method:"POST"}).validator(domainSchema).handler(async({data})=>{const c=await masterMutation();return createWhiteLabelDomain(c.sql,c.actorUserId,{...data,type:data.type as DomainType});});
export const updateMasterDomain=createServerFn({method:"POST"}).validator(updateDomainSchema).handler(async({data})=>{const c=await masterMutation();return updateWhiteLabelDomain(c.sql,c.actorUserId,{...data,type:data.type as DomainType});});
