...
const ASSIGN_PLAN_SQL = `with lock_scope as materialized (
  ...
  insert into public.store_subscriptions(
    tenant_id,store_id,tenant_plan_id,status,current_period_started_at,current_period_ends_at
  )
  ...
  from result r where exists(select 1 from created) returning id
)
...
export async function assignControlMerchantPlan(
  ...
  const rows = await sql.query(ASSIGN_PLAN_SQL, [tenantId, storeId, planId, useTrial, actorUserId]);
  const row = rows.at(0);
