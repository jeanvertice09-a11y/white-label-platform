...
function MasterDashboard() {
  ...
  return (
    <div className="master-stack master-dashboard">
      <MasterPageHeader
        title="Visão geral"
        description="Acompanhe operação, plataformas, lojas, receita e infraestrutura em um único lugar."
        action={<span className="master-live"><i />Dados reais da plataforma</span>}
      />
      ...
      <div className="master-analytics-grid">
        <MasterPanel title="Panorama da plataforma">
          <div className="master-panel-headline">
            <div><strong>Saúde operacional</strong><span>Comparativo dos principais ativos cadastrados</span></div>
            <span className="master-chip">Visão atual</span>
          </div>
        ...
        <MasterPanel title="Status das plataformas">
          <div className="master-panel-headline master-panel-headline--tight">
            <div><strong>Distribuição atual</strong><span>Baseada no status real dos tenants</span></div>
          </div>
          <StatusDonut active={data.metrics.activeTenants} total={data.metrics.tenants} />
          <div className="master-mini-stats">
            <div><span>Em trial</span><strong>{data.metrics.trialTenants}</strong></div>
            <div><span>Domínios</span><strong>{data.metrics.activeDomains}</strong></div>
          </div>
        </MasterPanel>
      </div>
        ...
        <MasterPanel title="Atividade recente">
          {data.audits.length ? <div className="master-list">{data.audits.slice(0, 7).map((item) => (
            <div className="master-list__row" key={item.id}>
              <div className="master-list__identity"><span className="master-list__dot" /><div><strong>{item.action}</strong><small>{item.resourceType}{item.resourceId ? ` · ${item.resourceId}` : ""}</small></div></div>
        ...
        <MasterPanel title="Requer atenção">
          {attention.length ? <div className="master-list">{attention.map((tenant) => (
            <div className="master-list__row" key={tenant.id}>
              <div className="master-list__identity"><span className="master-list__dot master-list__dot--warning" /><div><strong>{tenant.name}</strong><small>{tenant.slug}</small></div></div>
