...
import { addCartItem, getCatalogAdvancedSettings, getCatalogBehavior, getCatalogPublicMediaUrl, resolvePurchasableSelection } from "@white-label/catalog";
...
import { StorefrontTracking, trackStorefrontEvent } from "../features/storefront/storefront-tracking.tsx";
import { storefrontAdvancedTheme } from "../features/storefront/storefront-advanced-theme.ts";
import { storefrontResponsiveTheme } from "../features/storefront/storefront-responsive-theme.ts";
...
export const Route = createFileRoute("/produto/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const description = behavior?.showDescription ? data?.product.description ?? fallbackDescription : fallbackDescription;
    const image = data?.product.images.at(0); const ogImage = data && image ? getCatalogPublicMediaUrl(data.product, image.objectKey) : null;
    return { meta: [
      { title }, { name: "description", content: description },
      { property: "og:title", content: title }, { property: "og:description", content: description }, { property: "og:type", content: "product" },
