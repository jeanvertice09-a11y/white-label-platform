...
function VariantOptions(props: Readonly<{ variants: ProductVariant[]; selectedId: string; trackInventory: boolean; showPrice: boolean; onSelect: (id: string) => void }>): React.JSX.Element | null {
  ...
  return <fieldset className="sf__variants"><legend>Escolha uma opção</legend><div className="sf__variant-options">{props.variants.map((variant) => {
    const unavailable = props.trackInventory && variant.stockQuantity <= 0;
    const detail = unavailable ? "Indisponível" : props.showPrice ? storefrontMoney(variant.priceCents) : "Disponível";
    return <button type="button" key={variant.id} className="sf__variant" data-selected={props.selectedId === variant.id} data-unavailable={unavailable} disabled={unavailable} onClick={() => { props.onSelect(variant.id); }}><span>{variantLabel(variant)}</span><small>{detail}</small></button>;
  })}</div></fieldset>;
}

function Quantity(props: Readonly<{ value: number; max: number; disabled: boolean; onChange: (value: number) => void }>): React.JSX.Element {
  const upper = Math.max(1, props.max);
  return <div className="sf__quantity-block"><span>Quantidade</span><div className="sf__qty sf__qty--large"><button type="button" disabled={props.disabled || props.value <= 1} onClick={() => { props.onChange(Math.max(1, props.value - 1)); }} aria-label="Diminuir quantidade">−</button><output aria-live="polite">{props.value}</output><button type="button" disabled={props.disabled || props.value >= upper} onClick={() => { props.onChange(Math.min(upper, props.value + 1)); }} aria-label="Aumentar quantidade">+</button></div></div>;
}
...
export function ProductDetail(props: Readonly<ProductDetailProps>): React.JSX.Element {
  ...
  const quantityEnabled = props.quantityEnabled ?? true;
  const catalogOnly = props.catalogOnly ?? false;
  const sku = selectedVariant?.sku ?? props.product.sku;
  const purchaseVisible = showBuyButton && !catalogOnly;
  function selectVariant(nextId: string): void { setVariantId(nextId); setQuantity(1); setImageId(null); }
  const content = <section className={mode === "page" ? "sf__product-layout" : "sf__modal"} aria-modal={mode === "modal" ? "true" : undefined} role={mode === "modal" ? "dialog" : undefined}>
    {mode === "modal" ? <div className="sf__modal-head"><span /><button className="sf__close" type="button" onClick={props.onClose} aria-label="Fechar">×</button></div> : null}
    <div className="sf__detail"><Gallery product={props.product} images={images} selectedImageId={imageId} onImage={setImageId} /><div className="sf__panel">
      {props.categoryName ? <p className="sf__eyebrow">{props.categoryName}</p> : null}<h1 className="sf__product-title">{props.product.name}</h1>
      <Price current={currentPrice} compareAt={compareAt} show={props.showPrice} />{showSku && sku ? <p className="sf__meta">Código: {sku}</p> : null}{props.showShare ? <ShareActions title={props.product.name} url={props.shareUrl} /> : null}
      {showDescription && props.product.description ? <p className="sf__description">{props.product.description}</p> : null}<VariantOptions variants={props.product.variants} selectedId={variantId} trackInventory={props.product.trackInventory} showPrice={props.showPrice} onSelect={selectVariant} />
      {selectedVariant !== undefined && Object.keys(selectedVariant.attributes).length > 0 ? <dl className="sf__attributes">{Object.entries(selectedVariant.attributes).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl> : null}
      {props.showStock && props.product.trackInventory && hasValidSelection ? <p className="sf__stock" data-available={available}>{available ? `Disponível${limit < 10 ? ` · ${String(limit)} em estoque` : ""}` : "Sem estoque"}</p> : null}
      {purchaseVisible && quantityEnabled ? <Quantity value={quantity} max={limit} disabled={!available} onChange={setQuantity} /> : null}
      {purchaseVisible ? <button className="sf__primary sf__add" type="button" disabled={!available} onClick={() => { props.onAdd(props.product, selectedId, quantityEnabled ? quantity : 1); }}>{!hasValidSelection ? "Escolha uma opção" : available ? "Adicionar ao carrinho" : "Indisponível"}</button> : null}
      {catalogOnly ? <p className="sf__meta">Este catálogo está em modo vitrine.</p> : null}
    </div></div>
  </section>;
