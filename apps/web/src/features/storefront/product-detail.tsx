...
function Price(props: Readonly<{ current: number | null; compareAt: number | null; show: boolean }>): React.JSX.Element | null {
  ...
  return <div className="sf__price-line sf__price-line--detail">
...
function Gallery(props: Readonly<{
  product: Product;
  images: ProductImage[];
  ...
  return <div className="sf__gallery">
    <div className="sf__detail-image">
      {image === undefined || imageUrl === null
        ? <span className="sf__placeholder">Imagem não disponível</span>
        : <img src={imageUrl} alt={image.altText ?? props.product.name} />}
    </div>
    {props.images.length > 1 ? <div className="sf__thumbs" aria-label="Imagens do produto">
      {props.images.map((item) => <button type="button" key={item.id} data-active={item.id === image?.id} onClick={() => { props.onImage(item.id); }} aria-label="Ver imagem">
        <img src={getCatalogPublicMediaUrl(props.product, item.objectKey)} alt={item.altText ?? props.product.name} loading="lazy" />
      </button>)}
    </div> : null}
  </div>;
}

function VariantOptions(props: Readonly<{
  ...
  return <fieldset className="sf__variants">
    <legend>Escolha uma opção</legend>
    <div className="sf__variant-options">
      {props.variants.map((variant) => {
        const unavailable = props.trackInventory && variant.stockQuantity <= 0;
        return <button
          type="button"
          key={variant.id}
          className="sf__variant"
          data-selected={props.selectedId === variant.id}
          data-unavailable={unavailable}
          disabled={unavailable}
          onClick={() => { props.onSelect(variant.id); }}
        >
          <span>{variantLabel(variant)}</span>
          <small>{unavailable ? "Indisponível" : storefrontMoney(variant.priceCents)}</small>
        </button>;
      })}
    </div>
  </fieldset>;
...
function Quantity(props: Readonly<{ value: number; max: number; disabled: boolean; onChange: (value: number) => void }>): React.JSX.Element {
  ...
  return <div className="sf__quantity-block">
    <span>Quantidade</span>
    <div className="sf__qty sf__qty--large">
      <button type="button" disabled={props.disabled || props.value <= 1} onClick={() => { props.onChange(Math.max(1, props.value - 1)); }} aria-label="Diminuir quantidade">−</button>
      <output aria-live="polite">{props.value}</output>
      <button type="button" disabled={props.disabled || props.value >= upper} onClick={() => { props.onChange(Math.min(upper, props.value + 1)); }} aria-label="Aumentar quantidade">+</button>
    </div>
  </div>;
}
...
export function ProductDetail(props: Readonly<{
  ...
  const content = <section className={mode === "page" ? "sf__product-layout" : "sf__modal"} aria-modal={mode === "modal" ? "true" : undefined} role={mode === "modal" ? "dialog" : undefined}>
    ...
    <div className="sf__detail">
      <Gallery product={props.product} images={images} selectedImageId={imageId} onImage={setImageId} />
      <div className="sf__panel">
        {props.categoryName ? <p className="sf__eyebrow">{props.categoryName}</p> : null}
        <h1 className="sf__product-title">{props.product.name}</h1>
        <Price current={currentPrice} compareAt={compareAt} show={props.showPrice} />
        {props.product.description ? <p className="sf__description">{props.product.description}</p> : null}
        <VariantOptions variants={props.product.variants} selectedId={variantId} trackInventory={props.product.trackInventory} onSelect={selectVariant} />
        {selectedVariant !== undefined && Object.keys(selectedVariant.attributes).length > 0 ? <dl className="sf__attributes">
          {Object.entries(selectedVariant.attributes).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}
        </dl> : null}
        ...
        <Quantity value={quantity} max={limit} disabled={!available} onChange={setQuantity} />
        <button className="sf__primary sf__add" type="button" disabled={!available} onClick={() => { props.onAdd(props.product, selectedId, quantity); }}>
          {!hasValidSelection ? "Escolha uma opção" : available ? "Adicionar ao carrinho" : "Indisponível"}
        </button>
      </div>
    </div>
  </section>;
