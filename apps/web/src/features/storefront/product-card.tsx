...
import { getCatalogPublicMediaUrl, hasPromotionalPrice } from "@white-label/catalog";
...
export function ProductCard(props: Readonly<{ product: Product; categoryName: string | null; showPrice: boolean }>): React.JSX.Element {
  ...
  return <article className="sf__product-card"><a className="sf__product-link" href={storefrontProductPath(props.product.slug)} aria-label={`Ver ${props.product.name}`}><div className="sf__image">{imageUrl && image ? <img src={imageUrl} alt={image.altText ?? props.product.name} loading="lazy" /> : <span className="sf__placeholder">Imagem não disponível</span>}</div><div className="sf__card-body">{promotional && props.showPrice ? <PromotionBadge /> : null}{props.categoryName ? <span className="sf__meta">{props.categoryName}</span> : null}<h2 className="sf__product-name">{props.product.name}</h2>{props.showPrice ? <div className="sf__price-line">{promotional ? <span className="sf__old-price">{storefrontMoney(price.compareAt ?? 0)}</span> : null}<span className="sf__price">{price.prefix}{storefrontMoney(price.current)}</span></div> : null}<div className="sf__card-foot">{props.product.variants.length ? <span>{String(props.product.variants.length)} opções</span> : <span>Ver produto</span>}{!isAvailable ? <span className="sf__availability">Indisponível</span> : null}</div></div></a></article>;
}
