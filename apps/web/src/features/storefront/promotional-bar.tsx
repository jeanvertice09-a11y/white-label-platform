...
const CSS = `.sf-promo{background:var(--sf-primary);color:#fff;padding:.7rem 1rem;text-align:center;font:600 .9rem/1.35 var(--sf-font);display:flex;gap:.75rem;justify-content:center;align-items:center;flex-wrap:wrap}.sf-promo a{color:inherit;text-decoration:underline;text-underline-offset:3px}.sf-promo__count{font-variant-numeric:tabular-nums;white-space:nowrap;opacity:.92}@media(max-width:640px){.sf-promo{padding:.65rem .8rem;font-size:.82rem;gap:.4rem}.sf-promo__count{width:100%}}`;
...
export function PromotionalBar({
  merchandising,
}: Readonly<{ merchandising?: PublicCatalogMerchandising | null }>): React.JSX.Element | null {
  ...
  const copy = merchandising.href
    ? <a href={merchandising.href}>{merchandising.text}</a>
    : <span>{merchandising.text}</span>;
