...
function Categories(props: Readonly<{ categories: Category[]; activeId: string }>): React.JSX.Element | null {
  ...
  return <section className="sf__categories" id="categorias" aria-labelledby="sf-categories-title"><div className="sf__section-head"><div><span className="sf__eyebrow">Navegue</span><h2 id="sf-categories-title">Categorias</h2></div></div><div className="sf__category-list"><a className={!props.activeId ? "is-active" : ""} href="/">Todos os produtos</a>{props.categories.map((category) => <a className={props.activeId === category.id ? "is-active" : ""} key={category.id} href={storefrontCategoryPath(category.slug)}>{categoryName(props.categories, category.id)}</a>)}</div></section>;
}

function Toolbar(props: Readonly<{ data: StorefrontSnapshot; search: string; sort: Sort; onSearch: (value: string) => void; onSort: (value: Sort) => void }>): React.JSX.Element {
  return <div className="sf__toolbar">{props.data.settings.showSearch ? <label className="sf__search-wrap"><span className="sr-only">Buscar produtos</span><input className="sf__search" value={props.search} onChange={(event) => { props.onSearch(event.target.value); }} placeholder="Buscar na loja" aria-label="Buscar produtos" /></label> : <span />}<select className="sf__select sf__sort" value={props.sort} onChange={(event) => { props.onSort(event.target.value as Sort); }} aria-label="Ordenar produtos"><option value="position">Destaques</option><option value="name">Nome</option><option value="price_asc">Menor preço</option><option value="price_desc">Maior preço</option></select></div>;
}
...
function Pagination(props: Readonly<{ page: CatalogPage; loading: boolean; onPage: (page: number) => void }>): React.JSX.Element | null {
  ...
  return <nav className="sf__pagination" aria-label="Paginação dos produtos"><button type="button" disabled={props.loading || props.page.page <= 1} onClick={() => { props.onPage(props.page.page - 1); }}>Anterior</button><span>{props.page.page} de {pages}</span><button type="button" disabled={props.loading || props.page.page >= pages} onClick={() => { props.onPage(props.page.page + 1); }}>Próxima</button></nav>;
}
...
export function StorefrontView(props: Readonly<{ data: StorefrontSnapshot; initialCategoryId?: string; pageTitle?: string }>): React.JSX.Element {
  ...
  return <div className={`sf sf--${data.settings.layout}`} style={theme}><style>{storefrontTheme}</style><StoreHeader data={data} itemCount={itemCount} cartEnabled={cartEnabled} onCart={() => { setCartOpen(true); }} /><main className="sf__main">{banner ? <a className="sf__banner" href={banner.href ?? undefined}><img src={getCatalogPublicMediaUrl(banner,banner.imageObjectKey)} alt={banner.altText ?? banner.title ?? data.store.name} /></a> : null}<section className="sf__intro"><span className="sf__eyebrow">{props.pageTitle ? "Categoria" : "Loja online"}</span><h1>{props.pageTitle ?? data.store.name}</h1>{!props.pageTitle && data.profile?.description ? <p>{data.profile.description}</p> : !props.pageTitle && data.settings.labels["subtitle"] ? <p>{data.settings.labels["subtitle"]}</p> : null}{behavior.showShare ? <ShareActions title={props.pageTitle ?? data.store.name} /> : null}{behavior.catalogOnly ? <p className="sf__meta">Catálogo em modo vitrine.</p> : null}</section>{data.settings.showCategories ? <Categories categories={data.categories} activeId={listing.categoryId} /> : null}<section className="sf__products" id="produtos" aria-labelledby="sf-products-title"><div className="sf__section-head"><div><span className="sf__eyebrow">Catálogo</span><h2 id="sf-products-title">{props.pageTitle ? `Produtos em ${props.pageTitle}` : "Produtos"}</h2><p>{listing.page.total} item(ns) encontrado(s)</p></div></div><Toolbar data={data} search={listing.search} sort={listing.sort} onSearch={(value) => { listing.reset(() => { listing.setSearch(value); }); }} onSort={(value) => { listing.reset(() => { listing.setSort(value); }); }} /><Products data={data} page={listing.page} loading={listing.loading} error={listing.error} /><Pagination page={listing.page} loading={listing.loading} onPage={(next) => { listing.setPage((current) => ({ ...current, page: next })); }} /></section></main><StorefrontFooter storeName={data.store.name} profile={data.profile} />{cartOpen && cartEnabled ? <CartPanel cart={cart} whatsappEnabled={whatsappEnabled} showPrice={data.settings.showPrice} quantityEnabled={behavior.quantityEnabled} onChange={setCart} onClose={() => { setCartOpen(false); }} /> : null}</div>;
}
