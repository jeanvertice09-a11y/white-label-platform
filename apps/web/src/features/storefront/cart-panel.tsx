...
function CartRows(props: Readonly<{ cart: CartState; showPrice: boolean; quantityEnabled: boolean; onChange: (cart: CartState) => void }>): React.JSX.Element {
  ...
  return <div className="sf__cart-list">{props.cart.items.map((item) => <article className="sf__cart-row" key={item.productId + ":" + (item.variantId ?? "base")}>
    ...
    <div className="sf__cart-side">{props.showPrice ? <strong>{storefrontMoney(item.unitPriceCents * item.quantity)}</strong> : null}{props.quantityEnabled ? <div className="sf__qty"><button type="button" onClick={() => { change(item.productId, item.variantId, item.quantity - 1); }} aria-label="Diminuir quantidade">−</button><span>{item.quantity}</span><button type="button" onClick={() => { change(item.productId, item.variantId, item.quantity + 1); }} aria-label="Aumentar quantidade">+</button></div> : <span className="sf__meta">Qtd. {item.quantity}</span>}</div>
  </article>)}</div>;
}

function CheckoutFields(props: Readonly<{ name: string; phone: string; coupon: string; notes: string; onName: (value: string) => void; onPhone: (value: string) => void; onCoupon: (value: string) => void; onNotes: (value: string) => void }>): React.JSX.Element {
  return <div className="sf__checkout-fields"><div className="sf__checkout-section-head"><span>Seus dados</span><small>Usados para identificar o pedido.</small></div>
    ...
    <label className="sf__field"><span>Observação <small>opcional</small></span><textarea value={props.notes} maxLength={1000} onChange={(event) => { props.onNotes(event.target.value); }} placeholder="Alguma observação sobre o pedido?" /></label>
  </div>;
}

function OrderSuccess({ result, showPrice }: Readonly<{ result: CheckoutResult; showPrice: boolean }>): React.JSX.Element {
  return <div className="sf__success"><div className="sf__confirmation-head"><span className="sf__success-mark" aria-hidden="true">✓</span><div><span className="sf__eyebrow">Pedido registrado</span><h3>{result.displayNumber}</h3><p>Seu pedido já foi criado. O WhatsApp é o próximo passo para continuar o atendimento.</p></div></div>
    <div className="sf__order-items">{result.items.map((item, index) => <div className="sf__summary-row" key={`${item.productName}-${String(index)}`}><span>{item.quantity}× {item.productName}{item.variantName ? ` · ${item.variantName}` : ""}</span>{showPrice ? <strong>{storefrontMoney(item.totalCents)}</strong> : null}</div>)}</div>
    {showPrice ? <div className="sf__summary"><div className="sf__summary-row"><span>Subtotal</span><strong>{storefrontMoney(result.subtotalCents)}</strong></div>{result.discountCents > 0 ? <div className="sf__summary-row"><span>Desconto</span><strong>− {storefrontMoney(result.discountCents)}</strong></div> : null}<div className="sf__summary-row sf__summary-row--total"><span>Total</span><strong>{storefrontMoney(result.totalCents)}</strong></div></div> : null}<a className="sf__primary" href={result.whatsappUrl}>Continuar no WhatsApp</a>
  </div>;
}
...
export function CartPanel(props: Readonly<{ cart: CartState; whatsappEnabled: boolean; showPrice?: boolean; quantityEnabled?: boolean; onChange: (cart: CartState) => void; onClose: () => void }>): React.JSX.Element {
  ...
  const showPrice = props.showPrice ?? true; const quantityEnabled = props.quantityEnabled ?? true;
  async function checkout(): Promise<void> {
    setBusy(true); setMessage("");
    try {
      key.current ??= crypto.randomUUID();
      const result = await createWhatsappOrder({ data: { idempotencyKey: key.current, customerName: name.trim() || null, customerPhone: phone.trim() || null, couponCode: coupon.trim() || null, notes: notes.trim() || null, items: props.cart.items.map(({ productId, variantId, quantity }) => ({ productId, variantId, quantity })) } });
  ...
  return <div className="sf__overlay sf__overlay--drawer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}><section className="sf__modal sf__cart-modal" aria-modal="true" role="dialog" aria-label={success ? "Confirmação do pedido" : "Carrinho e checkout"}>
    ...
    {success ? <OrderSuccess result={success} showPrice={showPrice} /> : <><CartRows cart={props.cart} showPrice={showPrice} quantityEnabled={quantityEnabled} onChange={props.onChange} />{props.cart.items.length ? <div className="sf__checkout"><CheckoutFields name={name} phone={phone} coupon={coupon} notes={notes} onName={setName} onPhone={setPhone} onCoupon={setCoupon} onNotes={setNotes} />
      <div className="sf__summary">{showPrice ? <div className="sf__summary-row sf__summary-row--total"><span>Subtotal</span><strong>{storefrontMoney(cartTotalCents(props.cart))}</strong></div> : null}<p>Estoque, preço, cupom, desconto e total são validados novamente no servidor antes de o pedido ser criado.</p></div>{message ? <div className="sf__checkout-error" role="alert">{message}</div> : null}
