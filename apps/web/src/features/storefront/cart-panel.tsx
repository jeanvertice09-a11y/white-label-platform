...
export function CartPanel(props:Readonly<{cart:CartState;whatsappEnabled:boolean;onlineEnabled:boolean;showPrice?:boolean;quantityEnabled?:boolean;checkoutSettings:CheckoutSettings;onChange:(cart:CartState)=>void;onClose:()=>void}>):React.JSX.Element{
 ...
 async function checkout(kind: "whatsapp" | "pix"): Promise<void> {
  ...
  try {
   ...
   if (kind === "pix") {
    const result = await createOnlinePixOrder({ data: { ...common, payerEmail: email.trim(), shipping: shipping ? {...shipping,recipient:{postalCode,name:name.trim(),phone:phone.trim(),email:email.trim(),document,address:address.trim(),number:number.trim(),complement:complement.trim()||null,district:district.trim(),city:city.trim(),stateAbbr:stateAbbr.trim().toUpperCase()}} : null } });
