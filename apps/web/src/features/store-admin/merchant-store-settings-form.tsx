...
function StoreProfileFields(props: Readonly<{ draft: StoreDraft; setDraft: React.Dispatch<React.SetStateAction<StoreDraft>> }>): React.JSX.Element {
...
export function MerchantStoreSettingsForm(props: Readonly<{ name: string; profile: PublicStoreProfile }>): React.JSX.Element {
  ...
  async function submit(event: SyntheticEvent<HTMLFormElement>): Promise<void> {
    ...
    setStatus("");
    try {
      const result = await saveMerchantStoreProfile({ data: {
        name: draft.name,
  ...
  return (
    <form className="k-form" onSubmit={(event) => { void submit(event); }}>
      <StoreProfileFields draft={draft} setDraft={setDraft} />
