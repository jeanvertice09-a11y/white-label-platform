...
function OrdersFilters(props: Readonly<{
  search: string;
  status: "" | OrderStatus;
  date: string;
  loading: boolean;
      ...
      <div className="k-form__grid">
        ...
        <div className="k-field">
          ...
          <select
            ...
            value={props.status}
            onChange={(event) => {
              props.setStatus(event.target.value as "" | OrderStatus);
            }}
          >
            {statuses.map((value) => (
        ...
        <div className="k-field">
          <label htmlFor="orders-date">Data</label>
          <input
            id="orders-date"
            type="date"
            value={props.date}
  ...
  return (
    <div className="k-actions">
      ...
      <span className="k-status">
        Página {props.data.page} de {pages} · {props.data.total} pedido(s)
      </span>
      <button
        className="k-button"
        type="button"
...
export function OrdersList(
  { initialPage }: Readonly<{ initialPage: OrderPage }>,
): React.JSX.Element {
  ...
  return (
    <div className="k-stack">
      ...
      {!error && data.total > 0 ? (
        <OrdersPagination data={data} loading={loading} load={load} />
      ) : null}
    </div>
  );
}
