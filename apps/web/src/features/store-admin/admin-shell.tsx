...
function MobileNavigation({ onMenu }: Readonly<{ onMenu: () => void }>): React.JSX.Element {
  return (
    <nav className="k-admin__mobile-nav" aria-label="Atalhos do painel">
      ...
      <button type="button" onClick={onMenu} aria-controls="merchant-navigation"><DashboardIcon name="menu" /><span>Menu</span></button>
    </nav>
  );
}
...
export function AdminShell({ children }: Readonly<{ children: ReactNode }>): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    function onKeyDown(event: KeyboardEvent): void { if (event.key === "Escape") setMobileOpen(false); }
    ...
    <section className="k-admin">
      ...
      <div className="k-admin__main">
        <div className="k-admin__mobilebar">
          <button className="k-admin__menu" type="button" onClick={() => { setMobileOpen(true); }} aria-label="Abrir menu" aria-expanded={mobileOpen} aria-controls="merchant-navigation"><DashboardIcon name="menu" /></button>
      ...
      <MobileNavigation onMenu={() => { setMobileOpen(true); }} />
    </section>
  );
}
