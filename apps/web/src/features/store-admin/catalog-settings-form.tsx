...
function AppearanceFields(props: Readonly<{
  ...
  return <>
    <div className="k-field"><label>Layout</label><select value={draft.layout} onChange={(event) => { setField("layout", event.target.value as SettingsDraft["layout"]); }}><option value="classic">Clássico</option><option value="modern">Moderno</option></select></div>
    <div className="k-field"><label>Fonte</label><select value={draft.fontFamily} onChange={(event) => { setField("fontFamily", event.target.value as SettingsDraft["fontFamily"]); }}><option value="system">Sistema</option><option value="inter">Inter</option><option value="sans">Sem serifa</option><option value="serif">Com serifa</option></select></div>
...
function CatalogFields(props: Readonly<{
  ...
  return <>
    ...
    <div className="k-field"><label>Finalização do pedido</label><select value={draft.checkoutMode} onChange={(event) => { setField("checkoutMode", event.target.value as SettingsDraft["checkoutMode"]); }}><option value="whatsapp">WhatsApp</option><option value="online">Online (em preparação)</option><option value="both">WhatsApp + Online</option></select></div>
    ...
    <div className="k-field"><label>Título nos buscadores</label><input value={draft.seoTitle ?? ""} onChange={(event) => { setField("seoTitle", event.target.value || null); }} /></div>
    <div className="k-field"><label>Descrição nos buscadores</label><input value={draft.seoDescription ?? ""} onChange={(event) => { setField("seoDescription", event.target.value || null); }} /></div>
...
function Preview({ draft }: Readonly<{ draft: SettingsDraft }>): React.JSX.Element {
  ...
  return <div className="k-preview" style={style}><div className="k-preview__bar" /><div className="k-preview__body"><strong>Prévia {draft.layout === "modern" ? "Moderno" : "Clássico"}</strong><p className="k-muted">{draft.labels["subtitle"] || "Catálogo online"}</p><span className="k-preview__accent">Destaque da loja</span></div></div>;
}
