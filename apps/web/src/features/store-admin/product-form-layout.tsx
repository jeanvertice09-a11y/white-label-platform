...
function IdentityFields(props: Readonly<{
  draft: ProductDraft;
  setField: SetField;
}>): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <div className="k-field k-field--full">
        ...
        <textarea id="product-description" value={draft.description} onChange={(event) => { setField("description", event.target.value); }} placeholder="Descreva o produto com clareza." />
      </div>
    </EditorSection>
  );
}

function PricingFields(props: Readonly<{
  draft: ProductDraft;
  hasVariants: boolean;
  setField: SetField;
}>): React.JSX.Element {
  ...
  return (
    <EditorSection
      title="Preço"
      description={hasVariants ? "Valores base. Cada variante continua com seu próprio preço." : "Preço de venda, comparativo e custo interno."}
    >
      <div className="k-field">
...
function MainFields(props: Readonly<{
  ...
  return (
    <div className="k-editor-main">
      <IdentityFields draft={props.draft} setField={props.setField} />
      <PricingFields draft={props.draft} hasVariants={props.hasVariants} setField={props.setField} />
    </div>
  );
}

function PublicationFields(props: Readonly<{
  draft: ProductDraft;
  setField: SetField;
}>): React.JSX.Element {
  const { draft, setField } = props;
  return (
    <section className="k-context-section">
      ...
      <label className="k-switch-row">
        <span><strong>Produto ativo</strong><small>Visível quando as regras públicas permitirem.</small></span>
        <input type="checkbox" checked={draft.active} onChange={(event) => { setField("active", event.target.checked); }} />
      </label>
      <label className="k-switch-row">
        <span><strong>Controlar estoque</strong><small>Movimentações continuam registradas no ledger.</small></span>
        <input type="checkbox" checked={draft.trackInventory} onChange={(event) => { setField("trackInventory", event.target.checked); }} />
      </label>
    </section>
  );
}

function OrganizationFields(props: Readonly<{
  draft: ProductDraft;
  categories: Category[];
  setField: SetField;
}>): React.JSX.Element {
...
function StockFields(props: Readonly<{
  draft: ProductDraft;
  hasVariants: boolean;
}>): React.JSX.Element {
  return (
    <section className="k-context-section">
      ...
      <div className="k-context-value">
        ...
        <strong>{props.draft.stock}</strong>
      </div>
      <p className="k-muted">
        Ajuste o saldo em Estoque para preservar o histórico.
        {props.hasVariants ? " Variantes mantêm saldos próprios." : ""}
      </p>
    </section>
  );
}
...
function ContextFields(props: Readonly<{
  ...
  return (
    <aside className="k-editor-aside" aria-label="Publicação e organização">
      <PublicationFields draft={props.draft} setField={props.setField} />
      <OrganizationFields draft={props.draft} categories={props.categories} setField={props.setField} />
      <StockFields draft={props.draft} hasVariants={props.hasVariants} />
    </aside>
  );
}
...
export function ProductEditorLayout(props: Readonly<{
  ...
  return (
    <div className="k-editor-layout">
      <MainFields draft={props.draft} hasVariants={props.hasVariants} setField={props.setField} />
      <ContextFields draft={props.draft} categories={props.categories} hasVariants={props.hasVariants} setField={props.setField} />
    </div>
  );
}
