...
function FinancialEntriesSection({ state }: Readonly<{ state: MerchantFinanceState }>): React.JSX.Element {
  return <section className="k-workspace-section">
    ...
    {state.data.total>0?<div className="k-pagination"><span>{state.data.total} lançamento(s) · página {state.data.page} de {Math.max(1,Math.ceil(state.data.total/state.data.pageSize))}</span><div><button className="k-button" type="button" disabled={state.loading||state.data.page<=1} onClick={()=>{void reloadFinance(state,state.data.page-1).catch((cause:unknown)=>{state.setError(messageFrom(cause,"Não foi possível carregar o financeiro."));});}}>Anterior</button><button className="k-button" type="button" disabled={state.loading||state.data.page*state.data.pageSize>=state.data.total} onClick={()=>{void reloadFinance(state,state.data.page+1).catch((cause:unknown)=>{state.setError(messageFrom(cause,"Não foi possível carregar o financeiro."));});}}>Próxima</button></div></div>:null}
  </section>;
}
