...
import { getControlMerchant, searchControlMerchants } from "../../lib/server/control-merchants.functions.ts";
...
export function ControlMerchantsManager({ initial }: Readonly<{ initial: ControlMerchantWorkspace }>): React.JSX.Element {
  ...
  async function open(storeId: string): Promise<void> {
    try { const next = await getControlMerchant({ data: { storeId } }); if (!next) throw new Error("Loja não encontrada nesta White Label."); setDetail(next); setMessage(""); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Falha ao abrir lojista."); }
  }
  async function refreshDetail(): Promise<void> {
    if (!detail) return;
    setDetail(await getControlMerchant({ data: { storeId: detail.merchant.id } }));
    await refresh();
  }
  return <section className="control-plan-management-shell" id="merchant-management"><div className="control-plan-management"><header><span>Lojistas</span><h2>Gestão de lojas</h2><p>Cadastre lojistas, encontre rapidamente uma loja e abra o contexto completo de assinatura e operação.</p></header><ControlMerchantCreateForm plans={initial.plans} onCreated={() => refresh(1)} /><MerchantList list={list} query={query} status={status} message={message} onQuery={setQuery} onStatus={setStatus} onRefresh={(page) => { void refresh(page); }} onOpen={(id) => { void open(id); }} />{detail ? <ControlMerchantDetailPanel detail={detail} plans={initial.plans} onChanged={refreshDetail} onClose={() => { setDetail(null); }} /> : null}</div></section>;
}
