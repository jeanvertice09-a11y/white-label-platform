...
function ControlSidebar(props: Readonly<{
  data: ControlData;
  open: boolean;
  activeHref: string;
  onClose: () => void;
  ...
  return (
    <>
      <button
        type="button"
        className={props.open ? "control-overlay is-open" : "control-overlay"}
        onClick={props.onClose}
        aria-label="Fechar menu"
      />
      <aside className={props.open ? "control-sidebar is-open" : "control-sidebar"} aria-label="Navegação da White Label">
        <div className="control-brand">
          <span className="control-brand__mark" aria-hidden="true">{props.data.tenant.name.slice(0, 1).toUpperCase()}</span>
          <div><strong>{props.data.tenant.name}</strong><small>Operação da White Label</small></div>
        </div>
        ...
        <div className="control-sidebar__account">
          <span className="control-sidebar__avatar" aria-hidden="true">{props.data.tenant.name.slice(0, 1).toUpperCase()}</span>
          <div>
            <strong>{props.data.tenant.slug}</strong>
            <small>{props.data.tenant.status}</small>
          </div>
          <button
            type="button"
            className="control-sidebar__logout"
  ...
  return (
    <section className="control-section control-overview" id="overview">
      <div className="control-page-header">
        <div>
          <span className="console-page-kicker">{data.tenant.name}</span>
          <h1>Operação</h1>
          <p>Lojas, assinaturas, receita e infraestrutura da White Label em uma leitura única.</p>
        </div>
        <div className="control-page-context">
          <span className="console-status">{data.tenant.status}</span>
          <small>{data.tenant.slug}</small>
        </div>
      </div>

      <section className="control-overview-band" aria-label="Resumo operacional">
        <div className="control-overview-primary">
          <span>Receita recebida</span>
          <strong>{money(paid)}</strong>
          <small>{String(paidPayments.length)} pagamentos confirmados</small>
        </div>
        <div className="control-overview-stats">
      ...
      <div className="control-dashboard-columns">
        <section className="control-editorial-section control-operations-panel">
        ...
        <section className="control-editorial-section control-recent-panel">
          ...
          {data.stores.length ? (
            <div className="console-compact-list">
              {data.stores.slice(0, 5).map((store) => (
                <div className="console-compact-row" key={store.id}>
                  <div><strong>{store.name}</strong><small>{store.slug} · {String(store.memberCount)} membros</small></div>
                  <span>{store.status}</span>
                </div>
              ))}
            </div>
          ) : <div className="control-empty"><strong>Nenhuma loja ainda</strong><p>Cadastre o primeiro lojista para começar a operação.</p></div>}
        </section>
      </div>

      <section className="control-editorial-section control-finance-snapshot">
        <div className="control-editorial-section__header">
          <div><h2>Pagamentos recentes</h2><p>Movimentações registradas para esta White Label.</p></div>
          <a href="#tenant-billing">Ver faturamento</a>
        </div>
        {data.payments.length ? (
          <div className="control-table-wrap">
            <table className="control-table">
              <thead><tr><th>Data</th><th>Nível</th><th>Status</th><th>Valor</th></tr></thead>
              <tbody>{data.payments.slice(0, 6).map((item) => (
                <tr key={item.id}>
                  <td>{date(item.createdAt)}</td>
                  <td>{item.level}</td>
                  <td><span className="control-badge">{item.status}</span></td>
                  <td className="console-money">{money(item.amountCents)}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
        ) : <div className="control-empty"><strong>Sem pagamentos registrados</strong><p>As movimentações financeiras aparecerão aqui quando existirem.</p></div>}
      </section>
    </section>
  );
}
...
function BrandingSnapshot({ data }: Readonly<{ data: ControlData }>) {
  ...
  return (
    <section className="control-section control-branding-section" id="branding">
      ...
      <div className="control-branding-layout">
        <section className="control-branding-block">
          <div className="control-editorial-section__header"><div><h2>Marca</h2><p>Logo configurada no tenant.</p></div></div>
          <div className="console-brand-preview">
            {data.tenant.logoUrl ? <img className="control-logo" src={data.tenant.logoUrl} alt={`Logo de ${data.tenant.name}`} /> : <div className="control-empty"><strong>Logo não configurada</strong><p>A identidade aparecerá aqui quando houver uma URL de logo persistida.</p></div>}
          </div>
        </section>
        <section className="control-branding-block">
          <div className="control-editorial-section__header"><div><h2>Identidade visual</h2><p>Cor principal e configuração estruturada.</p></div></div>
    ...
    <div className="control-shell console-shell">
      ...
      <main className="control-main console-shell__content">
        <div className="control-mobile-bar">
          <button type="button" className="control-mobile-bar__menu" onClick={() => { setMobileOpen(true); }} aria-label="Abrir menu"><DashboardIcon name="menu" /></button>
          <div><strong>{data.tenant.name}</strong><span>Controle da White Label</span></div>
        </div>
