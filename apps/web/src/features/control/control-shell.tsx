...
function ControlSidebar({ data, open, onClose }: Readonly<{ data: TenantControlDashboardData; open: boolean; onClose: () => void }>): React.JSX.Element {
  ...
  return <>
    <button type="button" className={open ? "control-overlay is-open" : "control-overlay"} onClick={onClose} aria-label="Fechar menu" />
    <aside className={open ? "control-sidebar is-open" : "control-sidebar"} aria-label="Navegação da White Label">
