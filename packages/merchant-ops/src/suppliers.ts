  ...
  const rows = await sql.query(
    `select s.*,
     ...
     where s.tenant_id=$1 and s.store_id=$2
       and ($3::text is null
         or s.name ilike '%' || $3 || '%'
         or coalesce(s.trade_name,'') ilike '%' || $3 || '%'
         or coalesce(s.document,'') ilike '%' || $3 || '%'
         or coalesce(s.email,'') ilike '%' || $3 || '%')
     order by (s.status='active') desc,s.name asc,s.id asc
     limit $4 offset $5`,
...
export async function createSupplier(
  ...
  const rows = await sql.query(
    ...
    [
      ...
      scope.storeId,
      input.name.trim(),
      nullable(input.tradeName),
      nullable(input.document),
...
export async function updateSupplierStatus(
  ...
  assertScope(scope);
  const rows = await sql.query(
    `update public.merchant_suppliers
