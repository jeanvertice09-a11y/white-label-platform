...
import { assertCategoryInput, assertProductInput } from "./validation.ts";
...
export async function createProduct(
  ...
  const rows = await sql.query(
    ...
    [
      ...
      input.active, input.trackInventory, input.stockQuantity, input.position,
    ],
  );
...
export async function updateProduct(
  ...
  const rows = await sql.query(
    `update public.products set
       ...
       stock_quantity=$14,position=$15,updated_at=now()
...
export async function createVariant(
  ...
  const rows = await sql.query(
    ...
    [
      ...
      input.costCents, input.active, input.stockQuantity, input.position,
    ],
  );
...
export async function updateVariant(
  ...
  const rows = await sql.query(
    `update public.product_variants set
       ...
       cost_cents=$10,active=$11,stock_quantity=$12,position=$13,updated_at=now()
