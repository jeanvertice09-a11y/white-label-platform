...
function color(value: string | undefined, fallback: string): string {
  ...
  if (!/^#[0-9A-Fa-f]{6}$/.test(normalized)) throw new Error("Cor inválida");
...
function safeLink(value: string | null | undefined): string | null {
  ...
  if (url.protocol !== "https:") throw new Error("Link do banner deve usar HTTPS");
  return url.toString();
}
...
export function normalizeMerchantBannerInput(
  ...
  const expectedPrefix =
    `tenants/${scope.tenantId.toLowerCase()}/stores/${scope.storeId.toLowerCase()}/`;
